    ...
    public TypeResponse generateTravelType(TypeRequest request)
    {
        ...
        try {
            ...
            TravelType travelType = travelTypeRepository.findTravelTypeByCode(code)
                    .orElseThrow(()->new IllegalArgumentException("존재하지 않는 유형 코드입니다: " + code));

            return new TypeResponse(code, reason, keyWord,travelType.getImage(),travelType.getType_description(),travelType.getType_name(),travelType.getTrip_recommand());
        } catch (Exception e) {
            log.error("AI 응답 파싱 실패: {}", aiRawResponse, e);
            return new TypeResponse("ERROR", "AI 응답 파싱 실패", "","","","","");
        }
